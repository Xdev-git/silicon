    ...
    <main className="min-h-screen bg-white text-[#413F4C]">
      ...
      <section className="relative pt-32 pb-20 md:pt-44 md:pb-28 overflow-hidden" style={{ backgroundImage: "url('https://images.unsplash.com/photo-1486406146926-c627a92ad1ab?q=80&w=2070&auto=format&fit=crop')", backgroundSize: "cover", backgroundPosition: "center" }}>
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-12 relative z-10 text-center">
          <nav className="flex justify-center items-center text-xs text-white/50 mb-6 font-semibold uppercase tracking-widest">
            ...
            <span className="text-[#BC9F81]">FAQs</span>
          </nav>
          <p className="text-xs font-bold uppercase tracking-widest text-[#BC9F81] mb-4">Help Center</p>
          ...
          {faqs.map((category, catIndex) => (
            <div key={catIndex} className="mb-16 last:mb-0">
              <div className="flex items-center gap-4 mb-8">
                <div className="w-8 h-[2px] bg-[#BC9F81] hidden md:block"></div>
              ...
              <div className="space-y-3">
                {category.questions.map((faq, qIndex) => {
                  ...
                  return (
                    <div
                      key={qIndex}
                      className={`border overflow-hidden transition-all duration-300 ${isOpen ? 'bg-[#F3EEEB] border-[#BC9F81]/40 shadow-sm' : 'bg-white border-[#413F4C]/10 hover:border-[#413F4C]/30'}`}
                    >
                      <button
                      ...
                      >
                        <h3 className={`text-base md:text-lg font-bold transition-colors ${isOpen ? 'text-[#BC9F81]' : 'text-[#413F4C] group-hover/faq:text-[#BC9F81]'}`}>
                          {faq.q}
                        </h3>
                        <div className={`w-8 h-8 rounded-full border flex flex-shrink-0 items-center justify-center transition-all duration-300 ${isOpen ? 'bg-[#BC9F81] border-[#BC9F81] text-[#413F4C]' : 'border-[#413F4C]/20 text-[#413F4C]/50 bg-transparent'}`}>
      ...
      <section className="bg-[#F3EEEB] py-24 border-t border-[#413F4C]/10">
        <div className="max-w-4xl mx-auto px-6 md:px-12 text-center">
          ...
          <Link
            href="/contact"
            className="group relative inline-flex items-center justify-center bg-gradient-to-r from-[#BC9F81] to-[#AD9266] text-[#413F4C] font-bold py-4 px-10 uppercase tracking-widest text-xs overflow-hidden transition-all duration-300 hover:shadow-xl hover:scale-[1.02]"
          >
            Contact Us Today
          </Link>
        </div>
